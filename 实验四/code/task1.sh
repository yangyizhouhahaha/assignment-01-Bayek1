...
if [[ $1 == -h ]];then
	...
	echo 帮助信息;
	echo -h                          查看帮助信息;
	echo -f 文件名 -cr -size 长宽 -o 输出文件夹 对jpegpngsvg格式图片在保持原始宽高比的前提下压缩分辨率;
	echo -f 文件名 -w 文本水印 嵌入位置 -o 输出文件夹 对图片批量添加自定义文本水印;
	echo -f 文件名 -rn -f-b 前缀后缀 -o 输出文件夹 批量重命名统一添加文件名前缀或后缀;
	echo -f 文件名 -jpg -o 输出文件夹 将pngsvg图片统一转换为jpg格式图片;
